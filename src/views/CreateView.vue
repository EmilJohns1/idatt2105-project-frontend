<template>
  <div class="sidebar">
    <div id="questions">
      <div
        class="questionButtonDiv"
        v-for="(q, index) in questions"
        :key="index"
        @mouseover="handleOver(index)"
        @mouseleave="handleLeave(index)"
      >
        <a :href="'#q' + (index + 1)" :class="{ active: index === 0 }" class="questionButton"
          >Question {{ index + 1 }}</a
        >
  ...
  <div class="content">
    ...
    <div class="choice" v-if="selectedQuestionType === 'mc'">
      Alt 1: <input type="text" class="choiceInput" />
      <label class="container"
        ><input type="checkbox" checked /><span class="checkmark"></span></label
      ><br />
      Alt 2: <input type="text" class="choiceInput" />
      <label class="container"><input type="checkbox" /><span class="checkmark"></span></label
      ><br />
      Alt 3: <input type="text" class="choiceInput" />
      <label class="container"><input type="checkbox" /><span class="checkmark"></span></label
      ><br />
      Alt 4: <input type="text" class="choiceInput" />
      <label class="container"><input type="checkbox" /><span class="checkmark"></span></label
      ><br />
    </div>
    <div class="choice" id="tof" v-if="selectedQuestionType === 'tof'">
      ...
      True&nbsp;
      <label class="container"
        ><input type="radio" name="tof" checked /><span class="checkmark"></span></label
      ><br />
      False
      <label class="container"
        ><input type="radio" name="tof" /><span class="checkmark"></span></label
      ><br />
    </div>
...
const questions = ref<Question[]>([
  { question: '', imageUrl: null, maxPoints: 1, selectedQuestionType: 'mc' }
])

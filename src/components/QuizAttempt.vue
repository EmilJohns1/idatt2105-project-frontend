<template>
  <div class="container">
    ...
    <div v-if="quizAttempt.questionAttempts?.length" class="navButtons">
      <button
        v-if="currentQuestionIndex < quizAttempt.questionAttempts?.length - 1"
        class="nav nxt"
        @click="next()"
      >
        next
      ...
      <template v-if="currentQuestionAttempt.alternatives">
        <button
          ...
          :class="{
            alt: true,
            checkedFalse:
              alternative.wasCorrect !== alternative.wasSelected &&
              alternative.wasCorrect === false,
            uncheckedFalse:
              alternative.wasCorrect === alternative.wasSelected &&
              alternative.wasCorrect === false,
            checkedCorrect:
              alternative.wasCorrect === alternative.wasSelected && alternative.wasCorrect === true,
            uncheckedCorrect:
              alternative.wasCorrect !== alternative.wasSelected && alternative.wasCorrect === true
          }"
          :id="'button-' + index"
        ...
        <div id="tof-container">
          <button
            :class="{
              alt: true,
              checkedFalse:
                currentQuestionAttempt.correctAnswer === false &&
                currentQuestionAttempt.userAnswer === true,
              uncheckedFalse:
                currentQuestionAttempt.correctAnswer === false &&
                currentQuestionAttempt.userAnswer === false,
              checkedCorrect:
                currentQuestionAttempt.correctAnswer === true &&
                currentQuestionAttempt.userAnswer === true,
              uncheckedCorrect:
                currentQuestionAttempt.correctAnswer === true &&
                currentQuestionAttempt.userAnswer === false
  ...
  <div class="score-display">Score: {{ score }} / {{ quizAttempt.score }}</div>
</template>
...
const findPoints = async (alternatives: AlternativeRecord[]) => {
  ...
  alternatives.forEach((alternative) => {
    ...
    pointsPerAlternative =
      Math.floor((currentQuestionAttempt.value.points / correctAlternatives) * 100) / 100
  })
}
